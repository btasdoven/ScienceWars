...
{
    class DollyMinionMinion : BiologyMinion
    {
        private static float DEFAULT_EFFECT_COOLDOWN_TIME = 5.0f;
        private static int MAX_COPIES = 3;
        ...
        public DollyMinionMinion(Game game, Player ownerPlayer)
        ...
        public DollyMinionMinion(Game game, Player ownerPlayer,int  heritanceLevel)
            : base(game, ownerPlayer)
        {
            // butun initializationlari burada yap. yok efendim can setlemek, yok efendim movementspeed bilmemne.
                ...
                if (minionState == MinionState.ALIVE)
                {
                    ...
                    else
                    {
                        ...
                        if (heritanceLevel < 3 && numOfCopies < MAX_COPIES)
                        {
                            ...
                            {
                                effectCooldownTime = DEFAULT_EFFECT_COOLDOWN_TIME;
                                DollyMinionMinion newDolly = new DollyMinionMinion(this.game, this.ownerPlayer, heritanceLevel + 1);
